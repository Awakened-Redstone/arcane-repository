package dev.enjarai.arcane_repository.util.codec;
...
public interface CollectionCodec<E, C extends Collection<E>> extends Codec<C> {
    ...
    default <T> DataResult<Pair<C, T>> decode(final DynamicOps<T> ops, final T input) {
        return ops.getList(input).setLifecycle(Lifecycle.stable()).flatMap(stream -> {
            final CollectionCodec.DecoderState<E, C, T> decoder = getDecoderState(ops);
    ...
    @ApiStatus.Internal
    <T> CollectionCodec.DecoderState<E, C, T> getDecoderState(final DynamicOps<T> ops);
