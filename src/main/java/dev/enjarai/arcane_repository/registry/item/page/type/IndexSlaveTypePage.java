...
public class IndexSlaveTypePage extends IndexingTypePage {
    ...
    public LibraryIndex getMasterIndex(ItemStack book, World world, BlockPos pos) {
        var masterPos = getMasterPos(book); //       TODO: Why not use this? /\

        if (masterPos != null) {
            var masterEntity = world.getBlockEntity(masterPos);

            if (masterEntity instanceof MysticalLecternBlockEntity masterLectern) {
...
//    }
    ...
    public static class IndexSlaveLecternState extends PageLecternState {
        ...
        public void onUnload() {
            ...
            masterIndex.remove(index); // TODO: Idk if this works as intended, it seems it remove the linked master from itself when itself unloads, not when the master unloads
        }
