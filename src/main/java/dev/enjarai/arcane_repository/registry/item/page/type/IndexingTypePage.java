package dev.enjarai.arcane_repository.registry.item.page.type;
...
import static dev.enjarai.arcane_repository.registry.ModTags.INDEX_INTRACTABLE;
import static dev.enjarai.arcane_repository.registry.block.entity.MysticalLecternBlockEntity.LECTERN_DETECTION_RADIUS;
...
public class IndexingTypePage extends TypePageItem implements ItemInsertableTypePage {
    ...
    public IndexingTypePage(String id) {
        super(new ItemSettings().component(ModDataComponentTypes.LINKED_BLOCKS, List.of()), id);
    }
    ...
    public int getLinks(ItemStack book) {
        return book.getOrDefault(ModDataComponentTypes.LINKED_BLOCKS, List.of()).size();
    }
    ...
    private LibraryIndex getLinkedIndex(ItemStack book, World world, BlockPos pos) {
        ...
        var blockPosList = book.getOrDefault(ModDataComponentTypes.LINKED_BLOCKS, List.<BlockPos>of());
    ...
    public ActionResult book$useOnBlock(ItemUsageContext context) {
        ...
        if (isLinkableBlock(book, blockState) && context.getPlayer() != null && context.getPlayer().isSneaking()) {
            ...
            var librariesList = new ArrayList<>(book.getOrDefault(ModDataComponentTypes.LINKED_BLOCKS, List.of()));
            ...
            book.set(ModDataComponentTypes.LINKED_BLOCKS, librariesList);
    ...
    public void book$inventoryTick(ItemStack book, World world, Entity entity, int slot, boolean selected) {
        ...
        var positions = book.getOrDefault(ModDataComponentTypes.LINKED_BLOCKS, List.<BlockPos>of());
...
//    }
    ...
    public static abstract class IndexingAttributePage extends AttributePageItem {
        ...
        public void appendTooltip(ItemStack stack, @Nullable TooltipContext context, List<Text> tooltip, TooltipType type) {
            ...
            if (range != 1)
                tooltip.add(Text.translatable("item.arcane_repository.page.tooltip.type.indexing.range", range).formatted(Formatting.DARK_GREEN));
            if (links != 1)
                tooltip.add(Text.translatable("item.arcane_repository.page.tooltip.type.indexing.links", links).formatted(Formatting.DARK_GREEN));
            if (linkedRange != 1)
                tooltip.add(Text.translatable("item.arcane_repository.page.tooltip.type.indexing.linked_range", linkedRange).formatted(Formatting.DARK_GREEN));
        }
    }
}
