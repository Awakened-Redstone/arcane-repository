...
import dev.enjarai.arcane_repository.registry.item.ItemSettings;
...
public class ItemStorageTypePage extends TypePageItem implements ItemInsertableTypePage {
    ...
    private StorageFilterComponent getComponent(ItemStack stack) {
        return stack.get(ModDataComponentTypes.STORAGE_FILTERS);
    }
    ...
    public void onCraftToBook(ItemStack page, ItemStack book) {
        ...
        book.set(ModDataComponentTypes.PAGE_ATTRIBUTES, attributes);
    ...
    public void addFilteredItem(ItemStack book, Item item) {
        ...
        book.set(ModDataComponentTypes.STORAGE_FILTERS, component.withItems(newFilters));
    }
    ...
    public void clearFilteredItems(ItemStack book) {
        book.set(ModDataComponentTypes.STORAGE_FILTERS, getComponent(book).withItems(List.of()));
    }

    public void setFilteredItems(ItemStack book, List<Item> items) {
        var newFilters = ImmutableList.copyOf(items);

        book.set(ModDataComponentTypes.STORAGE_FILTERS, getComponent(book).withItems(newFilters));
    }
    ...
    public List<ItemStack> extractItems(IndexSource source, ExtractionRequest request, boolean apply) {
        if (request.isSatisfied()) {
            return Collections.emptyList();
        }

        ItemStack book = source.book();
        ...
        for (OverstackingStorageComponent.TypeStack typeStack : itemsList) {
            if (request.matches(typeStack.stack().getItem())) {
                ...
                if (apply) {
                    ...
                    if (itemCount <= extractAmount && source.getBlockEntity() instanceof IndexesBooks bookIndexer) {
                        int bookIndex = bookIndexer.arcane_repository$findIndex(book);
                        if (bookIndex > -1) {
                            bookIndexer.arcane_repository$removeFromIndex(typeStack.stack().getItem(), bookIndex);
                        }
                    }
                } else {
    ...
    public void saveOccupancy(ItemStack book, int stacks, int types) {
        book.set(ModDataComponentTypes.STORAGE_FILTERS, getComponent(book).withOccupiedStacks(stacks).withOccupiedTypes(types));
    }
    ...
    public void lectern$onPlaced(MysticalLecternBlockEntity lectern) {
        lectern.items = getFilteredItems(lectern.getBook()).stream()
          ...
          .collect(DefaultedList::of, DefaultedList::add, DefaultedList::addAll);
    }

    /**
     * Returns a set of types stored in the page,
     * these types don't need to match the class of the ones stored,
     * but instead a class that matches the one or search. <br/>
     * For example, item pages should return {@link  Item} instead of {@link ItemStack}
     * as the search doesn't care about components <br/>
     * <br/>
     * This is used for indexing
     *
     * @param book The book to get the types from
     */
    public Set<Item> getStoredTypes(ItemStack book) {
        ImmutableSet.Builder<Item> builder = ImmutableSet.builder();

        for (BigStack stack : getContents(book)) {
