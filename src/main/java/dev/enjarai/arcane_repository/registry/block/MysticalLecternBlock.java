package dev.enjarai.arcane_repository.registry.block;
...
public class MysticalLecternBlock extends LecternBlock {
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (world.getBlockEntity(pos) instanceof MysticalLecternBlockEntity lectern) {
            if (state.get(HAS_BOOK) && lectern.getBook().getItem() instanceof MysticalBookItem book) {
                return book.lectern$onUseWithItem(lectern, stack, state, world, pos, player, hit);
            }
        }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.MYSTICAL_LECTERN, MysticalLecternBlockEntity::serverTick);
    }
